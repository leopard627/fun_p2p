...
async def main():
    ...
    parser.add_argument(
        "--bootstrap", default="127.0.0.1:8468", help="Bootstrap node address:port"
    )
    ...
    create_parser.add_argument(
        "--announce", action="store_true", help="Announce wallet to the network"
    )
    ...
    backup_parser.add_argument(
        "--file", default="wallet_backup.txt", help="Backup file path"
    )
    ...
    fund_parser = subparsers.add_parser(
        "fund", help="Generate test funds (DEVELOPMENT ONLY)"
    )
    ...
    try:
        ...
        logger.info(
            f"✅ Connected to bootstrap node: {bootstrap_host}:{bootstrap_port}"
        )
    ...
    if args.command == "create":
        ...
        try:
            ...
            if wallet.balance < args.amount:
                logger.error(
                    f"❌ Insufficient balance: {wallet.balance} < {args.amount}"
                )
                return
        ...
        if wallets:
            ...
            for w in wallets:
                ...
                last_seen_str = (
                    f"{int(last_seen)}s ago"
                    if last_seen < 3600
                    else f"{int(last_seen/3600)}h ago"
                )
        ...
        try:
            with open(args.file, "w") as f:
                ...
                f.write(
                    f"Private Key: {base64.b64encode(wallet.private_key).decode('utf-8')}\n"
                )
                f.write(
            ...
            logger.info(
                "🔒 IMPORTANT: Store this file securely! Anyone with your private key can access your funds."
            )
        except Exception as e:
        ...
        if wallet.transactions:
            logger.info(
                f"Transaction history ({len(wallet.transactions)} transactions):"
            )
            for i, tx in enumerate(
                sorted(wallet.transactions, key=lambda x: x["timestamp"], reverse=True)
            ):
                timestamp = datetime.fromtimestamp(tx["timestamp"]).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                if tx["sender"] == wallet.address:
                    logger.info(
                        f"  {i+1}. SENT {tx['amount']} to {tx['recipient']} ({timestamp})"
                    )
                else:
                    logger.info(
                        f"  {i+1}. RECEIVED {tx['amount']} from {tx['sender']} ({timestamp})"
                    )
                if tx.get("memo"):
